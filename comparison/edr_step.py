    ...
    def build_graph(
        ...
        with tf.variable_scope("entity_type_classification"):
            ...
            word_hidden_states = tf.concat(
                [tf.zeros([total_sents, 1, de_out_dim]), hidden_states], axis=1
            )
...
if __name__ == "__main__":
    ...
    argdict = vars(args)
    name = "l2_{}_rnn_dim_{}_gcn_dim_{}_dep_proj_dim_{}_coe_{}_lr_{}".format(
        argdict["l2"],
        argdict["rnn_dim"],
        argdict["gcn_dim"],
    ...
    if args.command == "DP":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/edr_step:{}/model-{}".format(
                name, args.previous_model
            )
            ner_config = ner_train(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr_step:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_ = evaluate(
                Model(args), best_model_path, test_path, BATCH_SIZE
            )
            plotPRCurve(p_, r_, "./train_log/edr_step:{}".format(name))
            with open(
                "./train_log/edr_step:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr_step:{}/{}.txt".format(name, name),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(name + "\t")
                for model in [str(step * (3 + 1) + i * step) for i in range(3)]:
                    ...
                    for data in ["pn1", "pn2", "pn3"]:
                        ...
                        p100, p200, p300 = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/edr_step:{}/".format(name),
                                "model-" + model,
                            ),
                            data_path,
                            BATCH_SIZE,
