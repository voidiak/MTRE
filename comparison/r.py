...
def get_config(ds_train, ds_test, params):
    return TrainConfig(
        ...
        callbacks=[
            ...
            PeriodicTrigger(
                InferenceRunner(ds_test, [ScalarStats("total_loss")]), every_k_epochs=1
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-epochs", dest="epochs", default=5, type=int, help="training epochs"
    )
    parser.add_argument(
    ...
    if args.command == "train":
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/r:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_, p10_result, diff_result, wrong_pred = evaluate(
                Model(args), best_model_path, test_path, args.batch_size
            )
            plotPRCurve(p_, r_, "./train_log/r:{}".format(name))
            pickle.dump(
                {"precision": p_, "recall": r_},
                open("./train_log/r:{}/r_p_r.pkl".format(name), "wb"),
            )
            pickle.dump(
                diff_result, open("./train_log/r:{}/diff_list.pkl".format(name), "wb")
            )
            pickle.dump(
                wrong_pred,
                open("./train_log/r:{}/wrong_pred_list.pkl".format(name), "wb"),
            )
            with open(
                "./train_log/r:{}/{}.txt".format(name, "r_best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/r:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\n")
                for model in [
                    str((i + 1) * step) for i in range(args.epochs + args.add_epochs)
                ]:
                    f.write(model + "\t")
                    for data in ["pn1", "pn2", "test"]:
                        ...
                        p100, p200, p300 = evaluatepn(
                            Model(args),
                            os.path.join(
                                "./train_log/r:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
