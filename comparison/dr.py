    ...
    def build_graph(
        ...
        with tf.variable_scope("gcn_encoder") as scope:
            ...
            for l in range(self.gcn_layers):
                ...
                gAxW = tf.nn.relu(AxW)
                hidden_states_ = (
                    Dropout(gAxW, keep_prob=0.8) if l < self.gcn_layers - 1 else gAxW
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l2", dest="l2", default=1e-3, type=float, help="l2 regularization"
    )
    parser.add_argument(
    ...
    argdict = vars(args)
    name = "seed_{}_rnn_{}_gcn_{}".format(
        argdict["seed"], argdict["rnn_dim"], argdict["gcn_dim"]
    )
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/dr:{}/model-{}".format(name, args.previous_model)
            ...
            load_path = "./train_log/dr:{}/model-{}".format(name, current_step)
            resume_config = resume_train(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/dr:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_, p10_result = evaluate(
                Model(args), best_model_path, test_path, args.batch_size
            )
            plotPRCurve(p_, r_, "./train_log/dr:{}".format(name))
            pickle.dump(
                {"precision": p_, "recall": r_},
                open("./train_log/dr:{}/dr_p_r.pkl".format(name), "wb"),
            )
            with open(
                "./train_log/dr:{}/{}.txt".format(name, "dr_best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/dr:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\n")
                for model in [
                    ...
                    for data in ["test_r"]:
                        ...
                        p, r, f1, aur, p_, r_, p10_result = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/dr:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
