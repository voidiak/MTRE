...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pre_epochs", dest="pre_epochs", default=1, type=int, help="pretraining epochs"
    )
    parser.add_argument(
        "-epochs", dest="epochs", default=10, type=int, help="epochs to train/predict"
    )
    parser.add_argument(
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/er:{}/model-{}".format(name, args.previous_model)
            ...
            load_path = "./train_log/er:{}/model-{}".format(name, current_step)
            resume_config = resume_train(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/er:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_, p10_result = evaluate(
                Model(args), best_model_path, test_path, args.batch_size
            )
            plotPRCurve(p_, r_, "./train_log/er:{}".format(name))
            pickle.dump(
                {"precision": p_, "recall": r_},
                open("./train_log/er:{}/er_p_r.pkl".format(name), "wb"),
            )
            with open(
                "./train_log/er:{}/{}.txt".format(name, "er_best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/er:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\n")
                for model in [
                    ...
                    for data in ["pn1", "pn2", "test"]:
                        ...
                        p100, p200, p300 = evaluatepn(
                            Model(args),
                            os.path.join(
                                "./train_log/er:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
