...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pre_epochs", dest="pre_epochs", default=2, type=int, help="pretraining epochs"
    )
    parser.add_argument(
        "-epochs", dest="epochs", default=3, type=int, help="epochs to train/predict"
    )
    parser.add_argument(
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/edr_m:{}/model-{}".format(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr_m:{}/".format(name), "model-" + str(args.best_model)
            )
            ...
            pickle.dump(
                ...
                open("./train_log/edr_m:{}/p_r.pkl".format(name), "wb"),
            )
            pickle.dump(
                diff_result,
                open("./train_log/edr_m:{}/diff_list.pkl".format(name), "wb"),
            )
            with open(
                "./train_log/edr_m:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr_m:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\n")
                for model in [
                    ...
                    for data in ["pn1", "pn2", "test"]:
                        ...
                        p100, p200, p300 = evaluatepn(
                            Model(args),
                            os.path.join(
                                "./train_log/edr_m:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
