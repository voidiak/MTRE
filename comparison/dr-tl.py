    ...
    if args.command == "train":
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/dr-tl:{}/".format(name), "model-" + str(args.best_model)
            )
            ...
            pickle.dump(
                ...
                open("./train_log/dr-tl:{}/p_r.pkl".format(name), "wb"),
            )
            with open(
                "./train_log/dr-tl:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/dr-tl:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\n")
                for model in [
                    ...
                    for data in ["pn1", "pn2", "test"]:
                        ...
                        p100, p200, p300 = evaluatepn(
                            Model(args),
                            os.path.join(
                                "./train_log/dr-tl:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
