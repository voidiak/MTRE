...
class WarmupModel(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("dep_predictions"):
            ...
            arc_dep_hidden = tf.layers.conv1d(
                hidden_states,
                filters=self.params.projection_size,
                kernel_size=3,
                strides=1,
                padding="same",
            ...
            arc_head_hidden = tf.layers.conv1d(
                hidden_states,
                filters=self.params.projection_size,
                kernel_size=3,
                strides=1,
            ...
            root_score = tf.layers.dense(
                arc_head, self.num_dep_class, name="root_score"
            )
            ...
            dep_mask_ = tf.tile(
                tf.expand_dims(dep_mask, -1), [1, 1, self.num_dep_class]
            )
            dep_mask_ = tf.reshape(dep_mask_, [-1, mask_shape[1] * self.num_dep_class])
            dep_mask_ = tf.concat(
                [
                    tf.ones((mask_shape[0], 1)),
                    tf.zeros((mask_shape[0], self.num_dep_class - 1)),
                    dep_mask_,
                ],
                axis=1,
            )
        ...
        label_y = tf.one_hot(
            dep_y, (1 + seq_len) * 41, axis=-1, dtype=tf.int32, name="dep_label"
        )
            ...
            for model in [str(68700 + i * 4580) for i in range(5)]:
                ...
                for pnpath in ["./mdb/pn1.mdb", "./mdb/pn2.mdb", "./mdb/pn3.mdb"]:
                    p100, p200, p300 = predict(
                        Model(args),
                        os.path.join("./train_log/dr3/", "model-" + model),
                        pnpath,
                    )
                    logger.info(
                        "    {}:P@100:{}  P@200:{}  P@300:{}\n".format(
