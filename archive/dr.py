...
class Model(ModelDesc):
    ...
    def inputs(self):
        return [tf.TensorSpec([None, None], tf.int32, 'input_x'),#Xs
                ...
                tf.TensorSpec([None, None, None], tf.int32, 'dep_y'),#DepLabels
    ...
    def build_graph(self, input_x, input_pos1, input_pos2, dep_mask, x_len, seq_len, total_sents,\
        ...
        dep_ce = tf.nn.softmax_cross_entropy_with_logits_v2(logits=nn_dep_out, labels=dep_y)
        dp_loss = tf.reduce_sum(dep_mask * dep_ce) / tf.to_float(tf.reduce_sum(dep_mask))
        # tf.losses.add_loss(0.5*dp_loss)
        c=tf.get_variable('c',initializer=1,trainable=True)
        loss = re_loss + c * dp_loss
...
def resume_train(ds_train,ds_test,params):
    return AutoResumeTrainConfig(
        always_resume=False,
        data=QueueInput(ds_train),
        session_init=get_model_loader('./train_log/er/{}'.format(params.model)),
...
if __name__=='__main__':
    ...
    if args.command == 'train':
        ...
        with open('pn.txt', 'w', encoding='utf-8')as f:
            ...
            for model in ['160300', '146560', '151140', '155720', '160300', '164880', '169460', '174040', '178620',
                          '183200']:
                f.write(model + '\t')
                for pnpath in ['./mdb/pn1.mdb', './mdb/pn2.mdb', './mdb/pn3.mdb']:
                    p100, p200, p300 = predict(Model(args), os.path.join('./train_log/er/', 'model-' + model), pnpath)
