...
class DPModel(ModelDesc):
    ...
    def build_graph(
        ...
        accuracy = tf.cast(
            tf.equal(pred_masked, actual_masked), tf.float32, name="accu"
        )
...
if __name__ == "__main__":
    ...
    if args.command == "train_dp":
        ...
        with open(
            "./train_log/edr3/edrpn_{}_{}.txt".format(args.model, args.epochs),
            "w",
            encoding="utf-8",
        ) as f:
            ...
            for model in [str(args.model + i * 4580) for i in range(args.epochs)]:
                ...
                for pnpath in ["./mdb/pn1.mdb", "./mdb/pn2.mdb", "./mdb/pn3.mdb"]:
                    p100, p200, p300 = predict(
                        Model(args),
                        os.path.join("./train_log/edr3/", "model-" + model),
                        pnpath,
                    )
                    logger.info(
                        "    {}:P@100:{}  P@200:{}  P@300:{}\n".format(
