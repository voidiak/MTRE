...
def resume_train(ds_train, ds_test, params):
    return AutoResumeTrainConfig(
        ...
        session_init=get_model_loader(
            "./train_log/edr4:{}/model-{}".format(params.name, params.model)
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-coe", dest="coe", default=0.8, type=float, help="value for loss addition"
    )
    parser.add_argument(
    ...
    if args.command == "train":
        ...
        with open(
            "./train_log/edr4:{}/edr4pn_{}_{}.txt".format(
                args.name, args.model, args.epochs
            ),
            "w",
            encoding="utf-8",
        ) as f:
            ...
            for model in [str(args.model + i * 4580) for i in range(args.epochs)]:
                ...
                for pnpath in ["./mdb/pn1.mdb", "./mdb/pn2.mdb", "./mdb/pn3.mdb"]:
                    p100, p200, p300 = predict(
                        Model(args),
                        os.path.join(
                            "./train_log/edr4:{}/".format(args.name), "model-" + model
                        ),
                        pnpath,
                    )
                    logger.info(
