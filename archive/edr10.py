...
class WarmupModel(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("dep_predictions"):
            ...
            W = tf.get_variable(
                "shared_W",
                shape=[rnn_output_dim, 1, self.dep_proj_dim],
                initializer=tf.contrib.layers.xavier_initializer(),
            )
...
class Model(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("dep_predictions"):
            ...
            W = tf.get_variable(
                "shared_W", shape=[rnn_output_dim, 1, self.dep_proj_dim]
            )
            arc_dep = tf.tensordot(arc_dep_hidden, W, axes=[[-1], [0]])
            shape = tf.shape(arc_dep)
            arc_dep = tf.reshape(arc_dep, [shape[0], -1, self.dep_proj_dim])
...
if __name__ == "__main__":
    ...
    argdict = vars(args)
    name = "l2_{}_rnn_dim_{}_gcn_dim_{}_dep_proj_dim_{}_coe_{}_lr_{}_pre_epochs_{}_epochs_{}_{}".format(
        argdict["l2"],
        argdict["rnn_dim"],
        argdict["gcn_dim"],
