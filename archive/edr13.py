...
if __name__ == "__main__":
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/edr13:{}/model-{}".format(
            ...
            load_path = "./train_log/edr13:{}/model-{}".format(name, current_step)
            resume_config = resume_train(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr13:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_ = evaluate(
                Model(args), best_model_path, test_path, args.batch_size
            )
            plotPRCurve(p_, r_, "./train_log/edr13:{}".format(name))
            with open(
                "./train_log/edr13:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr13:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                f.write(name + "\t")
                for model in [
                    ...
                    for data in ["pn1", "pn2", "pn3"]:
                        ...
                        p100, p200, p300 = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/edr13:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
