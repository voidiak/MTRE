...
class WarmupModel(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("entity_fully_connected_layer"):
            ...
            b_e = tf.get_variable(
                "b", initializer=np.zeros([self.num_entity_class]).astype(np.float32)
            )
        ...
        with tf.variable_scope("dep_predictions"):
            ...
            arc_dep_hidden = tf.layers.dense(
                hidden_states, self.params.proj_dim, name="arc_dep_hidden"
            )
            arc_head_hidden = tf.layers.dense(
                hidden_states, self.params.proj_dim, name="arc_head_hidden"
            )
            # activation
            ...
            arc_head = tf.layers.dense(
                arc_head_hidden, self.params.dep_proj_dim, name="arc_head"
            )
            W = tf.get_variable(
                "shared_W",
                shape=[self.params.proj_dim, 1, self.params.dep_proj_dim],
                initializer=tf.contrib.layers.xavier_initializer(),
            )
    ...
    def optimizer(self):
        lr = tf.get_variable(
            "learning_rate", initializer=self.params.lr, trainable=False
        )
...
class Model(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("dep_predictions"):
            ...
            W = tf.get_variable(
                "shared_W", shape=[self.params.proj_dim, 1, self.params.dep_proj_dim]
            )
            arc_dep = tf.tensordot(arc_dep_hidden, W, axes=[[-1], [0]])
            shape = tf.shape(arc_dep)
            arc_dep = tf.reshape(arc_dep, [shape[0], -1, self.params.dep_proj_dim])
        ...
        with tf.variable_scope("fully_connected_layer") as scope:
            ...
            b = tf.get_variable(
                "b", initializer=np.zeros([self.num_class]).astype(np.float32)
            )
        ...
        loss = (1 - self.params.coe) * re_loss + self.params.coe * (
            0.25 * head_loss + 0.25 * tail_loss + 0.5 * dp_loss
        )
        if self.regularizer is not None:
            loss += tf.contrib.layers.apply_regularization(
                self.regularizer, tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
...
def resume_train(ds_train, ds_test, params):
    return AutoResumeTrainConfig(
        ...
        session_init=get_model_loader(
            "./train_log/edr5:{}/model-23448".format(params.name)
        ),
        starting_epoch=5,
        callbacks=[
            ModelSaver(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l2", dest="l2", default=0.0001, type=float, help="l2 regularization"
    )
    parser.add_argument(
        "-embed_loc",
        dest="embed_loc",
    ...
    if args.command == "pre_train":
        ...
        # predict
        with open(
            "./train_log/edr5:{}/edr5pn_{}.txt".format(args.name, args.epochs),
            "w",
            encoding="utf-8",
        ) as f:
            for model in [str(29310 + i * 5862) for i in range(args.epochs)]:
                ...
                for pnpath in ["./mdb/pn1.mdb", "./mdb/pn2.mdb", "./mdb/pn3.mdb"]:
                    p100, p200, p300 = predict(
                        Model(args),
                        os.path.join(
                            "./train_log/edr5:{}/".format(args.name), "model-" + model
                        ),
                        pnpath,
                    )
                    logger.info(
