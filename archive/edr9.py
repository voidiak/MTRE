...
if __name__ == "__main__":
    ...
    argdict = vars(args)
    name = "l2_{}_rnn_dim_{}_gcn_dim_{}_proj_dim_{}_dep_proj_dim_{}_coe_{}_lr_{}_pre_epochs_{}_epochs_{}_batch_size_{}".format(
        argdict["l2"],
        argdict["rnn_dim"],
        argdict["gcn_dim"],
        argdict["proj_dim"],
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/edr9:{}/model-{}".format(name, args.previous_model)
            ...
            load_path = "./train_log/edr9:{}/model-{}".format(name, current_step)
            resume_config = resume_train(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr9:{}/".format(name), "model-" + str(args.best_model)
            )
            p, r, f1, aur, p_, r_ = evaluate(
                Model(args), best_model_path, test_path, args.batch_size
            )
            plotPRCurve(p_, r_, "./train_log/edr9:{}".format(name))
            with open(
                "./train_log/edr6:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr9:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                for model in [
                    ...
                    for data in ["pn1", "pn2", "pn3"]:
                        ...
                        p100, p200, p300 = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/edr9:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
