...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l2", dest="l2", default=1e-5, type=float, help="l2 regularization"
    )
    parser.add_argument(
    ...
    if args.command == "pretrain":
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr7:{}/".format(name), "model-" + str(args.best_model)
            )
            ...
            with open(
                "./train_log/edr7:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr7:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                for model in [
                    ...
                    for data in ["pn1", "pn2", "pn3"]:
                        ...
                        p100, p200, p300 = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/edr7:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            args.batch_size,
                        )
