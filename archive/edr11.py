...
class WarmupModel(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("word_embedding"):
            ...
            word_embeded = tf.nn.embedding_lookup(
                word_embeddings, input_x, name="word_embeded"
            )
        ...
        summary.add_param_summary(("word_embeded", ["histogram"]))
        return loss
...
class Model(ModelDesc):
    ...
    def build_graph(
        ...
        with tf.variable_scope("entity_fully_connected_layer") as scope:
            w_e = tf.get_variable(
                "w",
                [self.gcn_dim, ENTITY_TYPE_CLASS],
                initializer=tf.contrib.layers.xavier_initializer(),
            )
            b_e = tf.get_variable(
                "b", initializer=np.zeros([ENTITY_TYPE_CLASS]).astype(np.float32)
...
if __name__ == "__main__":
    ...
    argdict = vars(args)
    name = "l2_{}_rnn_dim_{}_gcn_dim_{}_proj_dim_{}_dep_proj_dim_{}_coe_{}_lr_{}_pre_epochs_{}_epochs_{}_{}".format(
        argdict["l2"],
        argdict["rnn_dim"],
        argdict["gcn_dim"],
        argdict["proj_dim"],
    ...
    if args.command == "pretrain":
        ...
        if args.previous_model:
            ...
            load_path = "./train_log/edr10:{}/model-{}".format(
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join(
                "./train_log/edr10:{}/".format(name), "model-" + str(args.best_model)
            )
            ...
            with open(
                "./train_log/edr10:{}/{}.txt".format(name, "best_model"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
            ...
            with open(
                "./train_log/edr10:{}/{}.txt".format(name, name), "w", encoding="utf-8"
            ) as f:
                for model in [
                    ...
                    for data in ["pn1", "pn2", "pn3"]:
                        ...
                        p100, p200, p300 = evaluate(
                            Model(args),
                            os.path.join(
                                "./train_log/edr10:{}/".format(name), "model-" + model
                            ),
                            data_path,
                            BATCH_SIZE,
                        )
