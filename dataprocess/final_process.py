...
type2id = json.loads(open("./data/etype2id.json", encoding="utf-8").read())
entity2typeid = json.loads(open("./data/entity2id.json", encoding="utf8").read())
addentity = set()
    ...
    with open(file_path, encoding="utf-8") as f:
        for k, line in enumerate(f):
            ...
            for sent in bag["sentence"]:
                ...
                head_pos = setnullpos(head_pos)
                tail_pos = setnullpos(tail_pos)
                ...
                #         if dep_label[i]==40 or dep_head[i]>100 or dep_head[i]>sen_length:#dep label 缺失或越界
                #             dep_mask.append(0)
                #             dep_head[i] = 0#f过高的head值产生过高的dep label值
                #
                #         else:
                #             dep_mask.append(1)
                #
                #         dep.append(len(dep2id)*dep_head[i]+dep2id[dep_label[i]])#过高的dep label值在构造onehot label的时候发生数组越界
                if len(dep_head) == len(dep_label):
                    for i in range(sen_length):
                        if (
                            (dep_head[i] in entity_pos)
                            and dep_head[i] < len_limit
                            and dep_label[i] != 40
                        ):
            ...
            # add entity with empty type
            # when entity type not found error
            # try:
            #     for i in entity2typeid[bag['head_id']]:
            #         head_label[i]=1
            # except:
            #     addentity.add(bag['head_id'])
            # try:
            #     for i in entity2typeid[bag['tail_id']]:
            #         tail_label[i]=1
            # except:
            #     addentity.add(bag['tail_id'])
            # then go to addentity.py
            ...
            #     print('null sentences:{}'.format(null_sentence))
            # debug
            # if k>10:
            #     break
...
print("empty entity:{}".format(addentity))

# pickle.dump(addentity,open("empty_entity.pkl",'wb'))
...
for dtype in ["train", "test"]:
    for i in range(len(data[dtype]) - 1, -1, -1):
        ...
        for j in range(len(bag["wrds_list"]) - 1, -1, -1):
            ...
            data[dtype][i]["dep_mask_list"][j] = data[dtype][i]["dep_mask_list"][j][
                :MAX_WORDS
            ]
...
def procData(data, split="train"):
    ...
    for bag in data:
        res = {}  # k-hot label
        res["X"] = [
            [getId(wrd, voc2id, "UNK") for wrd in wrds] for wrds in bag["wrds_list"]
        ]
...
pickle.dump(param_data, open("params.pkl", "wb"))
print("writing data over")
...
pickle.dump(p1_data, open("pn1.pkl", "wb"))
pickle.dump(p2_data, open("pn2.pkl", "wb"))
pickle.dump(p3_data, open("pn3.pkl", "wb"))
