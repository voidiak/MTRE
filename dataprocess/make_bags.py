...
def readtraindata():
    with open("./data/riedel_train.json", "r",encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            train_data[_id]["rels"][relation2id.get(data["rel"],
                                                    relation2id["NA"])].append(
                                                        {"sent": clean_string(data["sent"]),"sent_dep":data["openie"]["sentences"][0]["basicDependencies"]})
            if i%10000==0:
...
def readtestdata():
    with open("./data/riedel_test.json","r",encoding='utf-8') as f:
        for i,line in enumerate(f):
            ...
            test_data[_id]["sents"].append({"sent":clean_string(data["sent"]),"sent_dep":data["openie"]["sentences"][0]["basicDependencies"]})
            if i%10000==0:
