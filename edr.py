    ...
    def __iter__(self):
        ...
        for _ in range(self.__len__()):
            ...
            if not self.isTrain:
                ...
                dropout = 0.8
                rec_dropout = 0.8
            yield [Xs,Pos1s,Pos2s,HeadPoss,TailPoss,DepMasks,X_len,max_seq_len,total_sents,total_bags,SentNum,ReLabels,\
                    DepLabels,HeadLabels,TailLabels,rec_dropout,dropout]
...
class Model(ModelDesc):
    ...
    def build_graph(self, input_x, input_pos1, input_pos2, head_pos, tail_pos, dep_mask, x_len, seq_len, total_sents,\
        ...
        d=tf.get_variable('d',initializer=0.5,trainable=True)
        loss = re_loss + c * dp_loss + d * (head_loss + tail_loss)
...
def predict(model,model_path,data_path):
    ...
    pred_config=PredictConfig(
        ...
        input_names=['input_x', 'input_pos1', 'input_pos2', 'head_pos', 'tail_pos', 'dep_mask', 'x_len', 'seq_len', 'total_sents',\
                    'total_bags', 'sent_num', 'input_y', 'dep_y', 'head_label', 'tail_label', 'rec_dropout', 'dropout'],
