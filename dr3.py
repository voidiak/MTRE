...
class WarmupModel(ModelDesc):
    ...
    def build_graph(self, input_x, input_pos1, input_pos2, dep_mask, x_len, seq_len, total_sents,\
        ...
        with tf.variable_scope('dep_predictions'):
            ...
            arc_dep_hidden=tf.layers.conv1d(hidden_states,filters=self.params.projection_size,kernel_size=3,strides=1,padding='same',kernel_initializer=tf.contrib.layers.xavier_initializer(),name='arc_dep_hidden')
            arc_head_hidden=tf.layers.conv1d(hidden_states,filters=self.params.projection_size,kernel_size=3,strides=1,padding='same',kernel_initializer=tf.contrib.layers.xavier_initializer(),name='arc_head_hidden')
        ...
        label_y=tf.one_hot(dep_y,(1 + seq_len) * 41,axis=-1,dtype=tf.int32,name='dep_label')
...
def resume_train(ds_train,ds_test,params):
    return AutoResumeTrainConfig(
        ...
        session_init=get_model_loader('./train_log/dr3/{}'.format(params.model)),
...
if __name__=='__main__':
    ...
    parser.add_argument('-projection_size', dest='projection_size', default=128, type=int,help='projection size for LSTMs and hidden layers')
    parser.add_argument('-depparse_projection_size', dest='depparse_projection_size', default=64, type=int,help='size of the representations used in the bilinear classifier for parsing')
    ...
    if args.command == 'train':
        ...
        with open('dr3pn3.txt','w',encoding='utf-8')as f:

            for model in [str(68700+i*4580) for i in range(5)]:
                f.write(model+'\t')
                for pnpath in ['./mdb/pn1.mdb','./mdb/pn2.mdb','./mdb/pn3.mdb']:
                    p100,p200,p300=predict(Model(args),os.path.join('./train_log/dr3/','model-'+model),pnpath)
