...
if __name__ == '__main__':
    ...
    if args.command == 'train':
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join('./train_log/er-tl:{}/'.format(name), 'model-' + str(args.best_model))
            ...
            pickle.dump({'precision': p_, 'recall': r_}, open('./train_log/er-tl:{}/p_r.pkl'.format(name), 'wb'))
            with open('./train_log/er-tl:{}/{}.txt'.format(name, 'best_model'), 'w', encoding='utf-8')as f:
                f.write('precision:\t{}\nrecall:\t{}\nf1:\t{}\nauc:\t{}\n'.format(p, r, f1, aur))
                f.write(name + '\n')
            ...
            with open('./train_log/er-tl:{}/{}.txt'.format(name, name), 'w', encoding='utf-8')as f:
                f.write(name + '\n')
                for model in [str((i+1) * step) for i in
                    ...
                    for data in ['pn1', 'pn2', 'test']:
                        data_path = './mdb100/{}.mdb'.format(data)
                        p100, p200, p300 = evaluatepn(Model(args), os.path.join('./train_log/er-tl:{}/'.format(name),
