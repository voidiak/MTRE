...
if __name__ == '__main__':
    ...
    parser.add_argument('-pre_epochs', dest='pre_epochs', default=2, type=int, help='pretraining epochs')
    ...
    if args.command == 'pretrain':
        ...
        if args.previous_model:
            ...
            load_path = './train_log/edr_m:{}/model-{}'.format(name, args.previous_model)
            ...
            load_path = './train_log/edr_m:{}/model-{}'.format(name, current_step)
        ...
        if args.best_model:
            ...
            best_model_path = os.path.join('./train_log/edr_m:{}/'.format(name), 'model-' + str(args.best_model))
            ...
            plotPRCurve(p_, r_, './train_log/edr_m:{}'.format(name))
            pickle.dump({'precision': p_, 'recall': r_}, open('./train_log/edr_m:{}/p_r.pkl'.format(name), 'wb'))
            pickle.dump(diff_result,open('./train_log/edr_m:{}/diff_list.pkl'.format(name), 'wb'))
            with open('./train_log/edr_m:{}/{}.txt'.format(name, 'best_model'), 'w', encoding='utf-8')as f:
                f.write('precision:\t{}\nrecall:\t{}\nf1:\t{}\nauc:\t{}\n'.format(p, r, f1, aur))
                f.write(name + '\n')
            ...
            with open('./train_log/edr_m:{}/{}.txt'.format(name, name), 'w', encoding='utf-8')as f:
                f.write(name + '\n')
                for model in [str(step * (args.pre_epochs + 1) + i * step) for i in
                    ...
                    for data in ['pn1', 'pn2', 'test']:
                        data_path = './mdb100/{}.mdb'.format(data)
                        p100, p200, p300 = evaluatepn(Model(args), os.path.join('./train_log/edr_m:{}/'.format(name),
